...
def load_config(file_path):
    try:
        with open(file_path, 'r') as file:
            ...
            # Replace placeholders with environment variables if using them
...
def get_user_selection(hosts, max_retries=3):
    ...
    while retries < max_retries:
        ...
        try:
            ...
            console.print(f"[bold red]Invalid input: {ve}[/bold red]. Please try again.")
            retries += 1
        ...
        with open(output_file, 'w') as f:
            for cmd in all_commands:
                try:
                    ...
                    logger.error(f"Error executing '{cmd}' on {device['host']}': {cmd_exc}")
...
def main():
    ...
    with Progress(
        ...
        task = progress.add_task("[green]Executing Commands...", total=len(selected_hosts))

        # Use ThreadPoolExecutor for parallel execution
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {executor.submit(execute_commands, host): host for host in selected_hosts}
            for future in as_completed(futures):
                progress.advance(task)

    logger.info("All tasks completed. Check the 'results' directory, CSV file, and 'network_automation.log' for details.")
    console.print(Panel.fit("[bold green]All tasks completed![/bold green]\nCheck the 'results' directory, CSV file, and 'network_automation.log' for details."))
